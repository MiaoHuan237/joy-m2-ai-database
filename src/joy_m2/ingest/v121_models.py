"""Immutable public authority models for Task 10A V1.21 candidates."""
...
_GENESIS_DIGEST = "331192032f184a44ed383e6dacc2f06fbacb60ad94414298fcb935ff56cb5906"
_BASELINE = {
    "release_version": "V1.20",
    "question_count": 543,
    "database_sha256": "b3e4911259fbc4063e788f418f6e52a53017ff079895bce679837914a5539292",
    "database_size_bytes": 9768960,
    "manifest_sha256": "19ac2e43ded74a3f3eaf75c184b5eb231c9f1b0b152f5914cd822a321b045098",
    "manifest_size_bytes": 6643,
...
_CANDIDATE_VIEWS = ("task11_candidate_questions_v121",)
_REPORT_STATUSES = {
...
def _require_batch_id(value: object, name: str = "batch_id") -> str:
    if type(value) is not str or _BATCH_ID.fullmatch(value) is None:
        raise PipelineError(f"{name} must be a canonical V1.21 batch identifier")
    return value
...
def _baseline_artifact(value: object, name: str) -> ArtifactRef:
    ...
    if (
        ...
        raise PipelineError(f"{name} does not match the V1.20 baseline authority")
    return artifact


def _validate_report_tuples(report: "V121ImportPreflightReport") -> None:
    for name in (
...
@dataclass(frozen=True)
class V121AdaptedImportPackage:
    package_root: Path
    manifest_path: Path
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.manifest) is not V121BatchImportManifest:
            raise PipelineError("manifest must be an exact V121BatchImportManifest")


@dataclass(frozen=True)
class V121ImportApproval:
    batch_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.target_release_version) is not str or self.target_release_version != "V1.21":
            raise ImportApprovalError("target_release_version must equal V1.21")
        expected = (
            f"USER APPROVED IMPORT BATCH {self.batch_id} {self.preflight_sha256} "
            f"V1.21 PARENT {self.parent_candidate_digest}"
        )
...
class V121BatchLedgerEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.approval) is not V121ImportApproval:
            raise PipelineError("approval must be an exact V121ImportApproval")
        if (
            self.approval.batch_id != self.batch_id
            or self.approval.preflight_sha256 != self.preflight_sha256
            or self.approval.target_release_version != "V1.21"
        ...
        if self.projected_question_count != 543 + self.cumulative_candidate_count:
            raise PipelineError("projected question count does not close")


@dataclass(frozen=True)
class V121EffectiveState:
    baseline_database: ArtifactRef
    candidate_digest: str
    batch_ledger: tuple[V121BatchLedgerEntry, ...]
    candidate_count: int
    projected_question_count: int
    ...
    def __post_init__(self) -> None:
        ...
        ledger = _exact_tuple(self.batch_ledger, V121BatchLedgerEntry, "batch_ledger")
        ...
        if not ledger:
            if (
                ...
                or self.projected_question_count != 543
            ):
                raise PipelineError("empty V1.21 state must be the fixed genesis state")
        else:
            previous_count = 0
    ...
    def __post_init__(self) -> None:
        for name, expected in _CONTRACT_VALUES.items():
            ...
            if type(value) is not type(expected) or value != expected:
                raise PipelineError(f"{name} does not match the V1.21 candidate contract")
        ...
        if tables != _CANDIDATE_TABLES or views != _CANDIDATE_VIEWS:
            raise PipelineError("candidate contract object tuples do not match V1.21")
        object.__setattr__(self, "required_candidate_tables", tables)
        object.__setattr__(self, "required_candidate_views", views)


@dataclass(frozen=True)
class V121PreflightRequest:
    manifest: V121BatchImportManifest
    package_root: Path
    baseline_database: ArtifactRef
    parent_candidate: V121CandidateVerificationRequest | None
    contract: V121CandidateContract
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "package_root", _path(self.package_root, "package_root"))
        _baseline_artifact(self.baseline_database, "baseline_database")
        if self.parent_candidate is not None and type(self.parent_candidate) is not V121CandidateVerificationRequest:
            raise PipelineError("parent_candidate must be V121CandidateVerificationRequest or None")
        if type(self.contract) is not V121CandidateContract:
            raise PipelineError("contract must be an exact V121CandidateContract")


@dataclass(frozen=True)
class V121ImportPreflightReport:
    batch_id: str
    status: str
    preflight_sha256: str
    manifest_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or type(self.baseline_question_count) is not int or self.baseline_question_count != 543
            or type(self.target_release_version) is not str or self.target_release_version != "V1.21"
        ):
        ...
        for name in count_names:
            ...
            if type(value) is not int or value < 0:
                raise PipelineError(f"{name} must be a non-negative integer")
        if self.before_count < 543:
            raise PipelineError("before_count cannot precede the V1.20 baseline")
...
@dataclass(frozen=True)
class V121ImportPreflightResult:
    manifest: V121BatchImportManifest
    effective_state: V121EffectiveState
    ...
    def __post_init__(self) -> None:
        if type(self.manifest) is not V121BatchImportManifest:
            raise PipelineError("manifest must be an exact V121BatchImportManifest")
        if type(self.effective_state) is not V121EffectiveState:
            raise PipelineError("effective_state must be an exact V121EffectiveState")
        if type(self.report) is not V121ImportPreflightReport:
            raise PipelineError("report must be an exact V121ImportPreflightReport")
        if (
...
@dataclass(frozen=True)
class V121ApprovedBatch:
    preflight_result: V121ImportPreflightResult
    package_root: Path
    approval: V121ImportApproval

    def __post_init__(self) -> None:
        if type(self.preflight_result) is not V121ImportPreflightResult:
            raise PipelineError("preflight_result must be an exact V121ImportPreflightResult")
        object.__setattr__(self, "package_root", _path(self.package_root, "package_root"))
        if type(self.approval) is not V121ImportApproval:
            raise PipelineError("approval must be an exact V121ImportApproval")
...
@dataclass(frozen=True)
class V121CandidateBuildRequest:
    approved_batches: tuple[V121ApprovedBatch, ...]
    output_dir: Path
    contract: V121CandidateContract

    def __post_init__(self) -> None:
        object.__setattr__(self, "approved_batches", _exact_tuple(
            self.approved_batches, V121ApprovedBatch, "approved_batches", nonempty=True,
        ))
...
@dataclass(frozen=True)
class V121CandidateVerificationRequest:
    candidate_dir: Path
    approved_batches: tuple[V121ApprovedBatch, ...]
    contract: V121CandidateContract
...
class V121CandidateArtifacts:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.state) is not V121EffectiveState:
            raise PipelineError("state must be an exact V121EffectiveState")
