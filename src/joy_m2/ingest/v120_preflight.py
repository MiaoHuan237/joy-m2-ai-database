"""V1.20 multi-batch import preflight boundary."""
...
__all__ = ("preflight_v120_import",)

_GENESIS = "4ff624aca875b0191fe8a617516d2919c6d700ce69c5a3b5d236adcf7ecc59f1"
_BASELINE_SHA = "5a7f1ca01c29dc638fb592c668ea9f597551f94d73001898587b187bdbe490ff"
_BASELINE_SIZE = 9_478_144
_BASELINE_MANIFEST_SHA = "cd048408df5e062592d5ed8553b3461860c876b0264660ab65c78c428584a510"
_BASELINE_MANIFEST_SIZE = 3_242
_BASELINE_RELEASE_DIGEST = "7246d099028e350ea5074524a808c9eb87e83e3df218349040eaf20f0109a69d"
_RAW_FIELDS = {
...
_FIXED_CONTRACT = {
    "profile": "V1.20", "baseline_release_version": "V1.19",
    "baseline_question_count": 502, "baseline_database_sha256": _BASELINE_SHA,
    ...
    "rollback_schema": "task10-v120-rollback-v1", "expected_user_version": 120,
    ...
    "image_root": "images/sha256", "required_baseline_view": "formal_complete_questions_v119",
...
_FIXED_MANIFEST = {
    ...
    "module": "M2", "target_release_version": "V1.20",
...
def _manifest_projection(manifest: V120BatchImportManifest) -> dict[str, object]:
    groups = {
...
def _validate_authority(
    ...
    if not _reconstructs_exactly(request, V120PreflightRequest):
        raise PipelineError("request must be an exact V120PreflightRequest")
    ...
    if not _reconstructs_exactly(request.manifest, V120BatchImportManifest):
    ...
    for name, expected in _FIXED_CONTRACT.items():
        if type(getattr(request.contract, name)) is not type(expected) or getattr(request.contract, name) != expected:
            raise PipelineError("contract does not match fixed V1.20 authority")
    for name, expected in _FIXED_MANIFEST.items():
        if type(getattr(request.manifest, name)) is not str or getattr(request.manifest, name) != expected:
            raise PipelineError("manifest does not match fixed V1.20 authority")
    ...
    if (
        ...
        raise PipelineError("baseline database does not match fixed V1.19 authority")
    if (
...
def _object_without_duplicates(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    if len(keys) != len(set(keys)):
        raise PipelineError("canonical V1.20 JSON contains duplicate keys")
    return dict(pairs)
...
def _manifest_from_package(root: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise PipelineError("canonical V1.20 manifest must be strict UTF-8 JSON") from exc
    expected = tuple(field.name for field in fields(V120BatchImportManifest))
    if type(payload) is not dict or tuple(payload) != expected:
        raise PipelineError("canonical V1.20 manifest has the wrong field envelope")
    return payload
...
def _load_package(
    manifest: V120BatchImportManifest, root: Path,
) -> tuple[
    dict[str, bytes],
...
def _formal_indexes(path: Path) -> tuple[dict[str, object], frozenset[str], frozenset[str]]:
    ...
    if len(data) != _BASELINE_SIZE or _sha(data) != _BASELINE_SHA:
        raise PipelineError("baseline database bytes do not match V1.19")
    ...
    try:
        ...
        if metadata != {"release_version": "V1.19", "schema_version": "task9-v119-formal-v1"}:
            raise PipelineError("baseline metadata does not match V1.19")
        rows = connection.execute(
            ...
            "primary_type,tags_json FROM formal_complete_questions_v119 ORDER BY formal_order"
        ).fetchall()
        if len(rows) != 502 or tuple(row[0] for row in rows) != tuple(range(1, 503)):
            raise PipelineError("formal V1.19 view must contain exact rows 1..502")
        ...
        if len(references) != 502:
            raise PipelineError("formal V1.19 question IDs must be unique")
        indexes = {
...
def _parent_context(
    ...
    tags: frozenset[str],
) -> tuple[V120EffectiveState, dict[str, object], frozenset[str], frozenset[str]]:
    parent = request.parent_candidate
    if parent is None:
        return (
            V120EffectiveState(request.baseline_database, _GENESIS, (), 0, 502),
            indexes,
            primary_types,
            tags,
        )
    if type(parent) is not V120CandidateVerificationRequest:
        raise PipelineError("parent candidate authority is invalid")
    verification = verify_v120_candidate(parent, config)
    if (
    ...
    ledger: list[V120BatchLedgerEntry] = []
    cumulative = 0
    ...
    confirmation = verify_v120_candidate(parent, config)
    if (
...
def _issue(
    ...
    if code not in _ISSUE_CODES:
        raise PipelineError("unapproved V1.20 issue code")
    candidate_id = (
...
def _report_values(
    manifest: V120BatchImportManifest, candidates: tuple[ImportCandidate, ...],
    ...
    missing_image_paths: tuple[str, ...], state: V120EffectiveState,
) -> dict[str, object]:
    ...
    return {
        ...
        "baseline_version": "V1.19", "baseline_release_digest": _BASELINE_RELEASE_DIGEST,
...
def _preflight_payload(
    request: V120PreflightRequest, candidates: tuple[ImportCandidate, ...],
    ...
    report: dict[str, object], state: V120EffectiveState,
) -> dict[str, object]:
...
def preflight_v120_import(
    ...
    """Validate one canonical package against formal V1.19 without writing."""
    ...
    return V120ImportPreflightResult(request.manifest, state, candidates, issues, report)
