"""Strict V1.20 canonical package-manifest boundary."""
...
from .v120_models import V120BatchImportManifest


__all__ = ("load_v120_import_manifest",)
...
def load_v120_import_manifest(path: Path) -> V120BatchImportManifest:
    """Load one strict V1.20 package manifest from its canonical package root."""
    ...
    expected_fields = tuple(field.name for field in fields(V120BatchImportManifest))
    ...
    try:
        return V120BatchImportManifest(**values)
    except (TypeError, PipelineError) as exc:
        raise PipelineError("manifest does not satisfy V1.20 authority") from exc
