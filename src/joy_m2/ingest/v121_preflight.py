"""V1.21 multi-batch import preflight boundary."""
...
__all__ = ("preflight_v121_import",)

_GENESIS = "331192032f184a44ed383e6dacc2f06fbacb60ad94414298fcb935ff56cb5906"
_BASELINE_SHA = "b3e4911259fbc4063e788f418f6e52a53017ff079895bce679837914a5539292"
_BASELINE_SIZE = 9_768_960
_BASELINE_MANIFEST_SHA = "19ac2e43ded74a3f3eaf75c184b5eb231c9f1b0b152f5914cd822a321b045098"
_BASELINE_MANIFEST_SIZE = 6_643
_BASELINE_RELEASE_DIGEST = "1eb046cd247c362ab6b6052ca7fecddea914340dd7421bf136012a9a086c9fcf"
_RAW_FIELDS = {
...
_FIXED_CONTRACT = {
    "profile": "V1.21", "baseline_release_version": "V1.20",
    "baseline_question_count": 543, "baseline_database_sha256": _BASELINE_SHA,
    ...
    "rollback_schema": "task11-v121-rollback-v1", "expected_user_version": 121,
    ...
    "image_root": "images/sha256", "required_baseline_view": "formal_complete_questions_v120",
...
_FIXED_MANIFEST = {
    "schema_version": "task11-v121-import-manifest-v1", "project": "Joy M2 AI Database",
    "module": "M2", "target_release_version": "V1.21",
...
def _manifest_projection(manifest: V121BatchImportManifest) -> dict[str, object]:
    groups = {
...
def _validate_authority(
    ...
    if not _reconstructs_exactly(request, V121PreflightRequest):
        raise PipelineError("request must be an exact V121PreflightRequest")
    if not _reconstructs_exactly(config, PipelineConfig):
        raise PipelineError("config must be an exact PipelineConfig")
    if not _reconstructs_exactly(request.contract, V121CandidateContract):
        raise PipelineError("contract carrier is invalid")
    if not _reconstructs_exactly(request.manifest, V121BatchImportManifest):
    ...
    for name, expected in _FIXED_CONTRACT.items():
        if type(getattr(request.contract, name)) is not type(expected) or getattr(request.contract, name) != expected:
            raise PipelineError("contract does not match fixed V1.21 authority")
    for name, expected in _FIXED_MANIFEST.items():
        if type(getattr(request.manifest, name)) is not str or getattr(request.manifest, name) != expected:
            raise PipelineError("manifest does not match fixed V1.21 authority")
    ...
    if (
        ...
        raise PipelineError("baseline database does not match fixed V1.20 authority")
    if (
...
def _object_without_duplicates(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    if len(keys) != len(set(keys)):
        raise PipelineError("canonical V1.21 JSON contains duplicate keys")
    return dict(pairs)
...
def _manifest_from_package(root: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise PipelineError("canonical V1.21 manifest must be strict UTF-8 JSON") from exc
    expected = tuple(field.name for field in fields(V121BatchImportManifest))
    if type(payload) is not dict or tuple(payload) != expected:
        raise PipelineError("canonical V1.21 manifest has the wrong field envelope")
    return payload
...
def _load_package(
    manifest: V121BatchImportManifest, root: Path,
) -> tuple[
    dict[str, bytes],
...
def _formal_indexes(path: Path) -> tuple[dict[str, object], frozenset[str], frozenset[str]]:
    ...
    if len(data) != _BASELINE_SIZE or _sha(data) != _BASELINE_SHA:
        raise PipelineError("baseline database bytes do not match V1.20")
    ...
    try:
        ...
        if metadata != {"release_version": "V1.20", "schema_version": "task10-v120-formal-v1"}:
            raise PipelineError("baseline metadata does not match V1.20")
        rows = connection.execute(
            ...
            "primary_type,tags_json FROM formal_complete_questions_v120 ORDER BY formal_order"
        ).fetchall()
        if len(rows) != 543 or tuple(row[0] for row in rows) != tuple(range(1, 544)):
            raise PipelineError("formal V1.20 view must contain exact rows 1..543")
        ...
        if len(references) != 543:
            raise PipelineError("formal V1.20 question IDs must be unique")
        indexes = {
...
def _parent_context(
    ...
    tags: frozenset[str],
) -> tuple[V121EffectiveState, dict[str, object], frozenset[str], frozenset[str]]:
    parent = request.parent_candidate
    if parent is None:
        return (
            V121EffectiveState(request.baseline_database, _GENESIS, (), 0, 543),
            indexes,
            primary_types,
            tags,
        )
    if type(parent) is not V121CandidateVerificationRequest:
        raise PipelineError("parent candidate authority is invalid")
    verification = verify_v121_candidate(parent, config)
    if (
    ...
    ledger: list[V121BatchLedgerEntry] = []
    cumulative = 0
    ...
    expected_counts = {
        ...
        "projected_question_count": 543 + cumulative,
    }
    ...
    try:
        ...
        ledger_rows = tuple(connection.execute(
            ...
            "FROM task11_v121_batch_ledger_v1 ORDER BY batch_ordinal"
        ))
        ...
        rows = tuple(connection.execute(
            ...
            "FROM task11_v121_candidates_v1 ORDER BY aggregate_order"
        ))
    except PipelineError:
        raise
    ...
    confirmation = verify_v121_candidate(parent, config)
    if (
...
def _issue(
    ...
    if code not in _ISSUE_CODES:
        raise PipelineError("unapproved V1.21 issue code")
    candidate_id = (
...
def _report_values(
    manifest: V121BatchImportManifest, candidates: tuple[ImportCandidate, ...],
    ...
    missing_image_paths: tuple[str, ...], state: V121EffectiveState,
) -> dict[str, object]:
    ...
    return {
        ...
        "baseline_version": "V1.20", "baseline_release_digest": _BASELINE_RELEASE_DIGEST,
        "baseline_question_count": 543, "target_release_version": "V1.21",
...
def _preflight_payload(
    request: V121PreflightRequest, candidates: tuple[ImportCandidate, ...],
    ...
    report: dict[str, object], state: V121EffectiveState,
) -> dict[str, object]:
    manifest = request.manifest
    return {
        "schema": "task11-v121-preflight-v1", "batch_id": manifest.batch_id,
...
def preflight_v121_import(
    request: V121PreflightRequest,
    config: PipelineConfig,
) -> V121ImportPreflightResult:
    """Validate one canonical package against formal V1.20 without writing."""
    ...
    report = V121ImportPreflightReport(
        preflight_sha256=_sha(_canonical(payload) + b"\n"), **report_values,
    )
    return V121ImportPreflightResult(request.manifest, state, candidates, issues, report)
