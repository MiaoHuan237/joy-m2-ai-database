"""Independent read-only V1.21 candidate verification boundary."""
...
from .v121_writer_profiles import (
...
def _nested_authority_valid(batch: object) -> bool:
    try:
        if not _reconstructs_exactly(batch, V121ApprovedBatch):
            return False
        ...
        return (
            _reconstructs_exactly(batch.approval, V121ImportApproval)
            and _reconstructs_exactly(result, V121ImportPreflightResult)
            and _reconstructs_exactly(manifest, V121BatchImportManifest)
            and _reconstructs_exactly(state, V121EffectiveState)
            and _reconstructs_exactly(report, V121ImportPreflightReport)
            ...
            and all(
                _reconstructs_exactly(item, V121BatchLedgerEntry)
                and _reconstructs_exactly(item.approval, V121ImportApproval)
...
def _preflight_authority_closed(approved: V121ApprovedBatch) -> bool:
    ...
    return (
        ...
        and report.baseline_version == "V1.20"
        and report.baseline_release_digest == BASELINE_RELEASE_DIGEST
        and report.baseline_question_count == 543
        and report.target_release_version == "V1.21"
...
def _preflight_payload(approved: V121ApprovedBatch) -> dict[str, object]:
    result = approved.preflight_result
    manifest = result.manifest
    groups = (
    ...
    return {
        "schema": "task11-v121-preflight-v1",
        "batch_id": manifest.batch_id,
...
def _package_evidence_valid(approved: V121ApprovedBatch) -> bool:
    try:
...
def _candidate_identity(
    ...
    for ordinal, approved in enumerate(approved_batches, start=1):
        ...
        for order, candidate in enumerate(result.candidates):
            ...
            candidates.append({
                "aggregate_order": 543 + prior + order + 1,
                "batch_ordinal": ordinal,
                "batch_id": approval.batch_id,
    ...
    payload = {
        "schema": "task11-v121-candidate-identity-v1",
        "baseline": _BASELINE,
        "target_release_version": "V1.21",
        "batch_ledger": ledger,
    ...
    try:
        ...
        for ordinal, approved in enumerate(approved_batches, start=1):
            ...
            expected_statement = (
                ...
                f"{approval.preflight_sha256} V1.21 PARENT {approval.parent_candidate_digest}"
            )
            valid["approval"] &= (
                approval.batch_id == result.manifest.batch_id
                and approval.preflight_sha256 == result.report.preflight_sha256
                and approval.target_release_version == "V1.21"
            ...
            valid["parent"] &= (
                ...
                and result.effective_state.projected_question_count == 543 + prior_count
                ...
                and result.report.before_count == 543 + prior_count
            )
            ...
            authority[f"{directory}/approval.json"] = canonical_json_file_bytes({
                "schema_version": "task11-v121-import-approval-v1",
                "batch_id": approval.batch_id,
                "preflight_sha256": approval.preflight_sha256,
                "target_release_version": "V1.21",
                "parent_candidate_digest": approval.parent_candidate_digest,
            ...
            prefix = {
                ...
                "counts": {
                    ...
                    "projected_question_count": 543 + prior_count,
                },
            }
...
def _formal_baseline_valid(
    ...
    try:
        ...
        expected_sums = (
            f"{BASELINE_SHA256}  Joy_M2_Complete_Question_DB_V1_20.sqlite3\n"
        ...
        return (
            ...
            and parsed.get("release_version") == "V1.20"
            and parsed.get("release_status") == "published"
            and parsed.get("database_schema") == "task10-v120-formal-v1"
    ...
    while current != staging_root and current.is_relative_to(staging_root):
        ...
        if manifest.is_file() and database.is_file():
            ...
            if (
                ...
                and payload.get("release_version") == "V1.21"
...
def _sqlite_expected(
    ...
    for ordinal, approved in enumerate(approved_batches, start=1):
        ...
        cumulative += len(result.candidates)
        ledgers.append((
            ordinal, approval.batch_id, "V1.21", "task11-v121-candidate-v1",
            approval.parent_candidate_digest, approval.preflight_sha256,
            result.report.manifest_sha256, approval.statement, "V1.20",
            BASELINE_SHA256, BASELINE_RELEASE_DIGEST, 543, len(result.candidates),
            cumulative, 543 + cumulative, "candidate",
        ))
        for order, candidate in enumerate(result.candidates):
...
def _actual_collision_free(
    ...
    try:
        baseline_rows = tuple(database.execute(
            ...
            "FROM formal_complete_questions_v120 ORDER BY formal_order"
        ))
        candidate_rows = tuple(database.execute(
            ...
            "image_roles_json FROM task11_v121_candidates_v1 ORDER BY aggregate_order"
        ))
...
def _sqlite_checks(
    ...
    try:
        ...
        expected_task11 = {}
        for sql in TASK11_SCHEMA_SQL:
            words = sql.split()
            kind = words[1].lower()
            name = words[2]
            expected_task11[name] = (kind, sql)
        actual_task11 = {
            name: row for name, row in candidate_master.items()
            if name.startswith("task11_")
        }
        state["sqlite_schema"] = (
            candidate.execute("PRAGMA user_version").fetchone() == (121,)
            and set(actual_task11) == set(expected_task11)
            and all(
                actual_task11[name][0] == kind
                and normalized_sql(actual_task11[name][3]) == normalized_sql(sql)
                for name, (kind, sql) in expected_task11.items()
            )
            and all(candidate_master.get(name) == row for name, row in baseline_master.items())
            and set(candidate_master) == set(baseline_master) | set(expected_task11)
        )
        preserved_names = tuple(
            name for name, row in baseline_master.items() if row[0] in {"table", "view"}
        )
        state["v120_preservation"] = all(
        ...
        actual_ledger = tuple(candidate.execute(
            "SELECT * FROM task11_v121_batch_ledger_v1 ORDER BY batch_ordinal"
        ))
        ...
        actual_images = tuple(candidate.execute(
            "SELECT * FROM task11_v121_images_v1 ORDER BY candidate_relative_path,"
            "batch_ordinal,batch_id,proposed_question_id,image_order"
        ))
        actual_taxonomy = tuple(candidate.execute(
            "SELECT * FROM task11_v121_taxonomy_v1 ORDER BY batch_ordinal,batch_id,"
        ...
        state["candidate_projection"] = (
            ...
            and tuple(candidate.execute(
                "SELECT * FROM task11_candidate_questions_v121 ORDER BY formal_order"
            ))[:543]
        ...
        state["count_closure"] = (
            ...
            and candidate.execute(
                "SELECT COUNT(*) FROM task11_v121_candidates_v1"
...
def verify_v121_candidate(
    ...
    """Verify one V1.21 staging generation without mutating any input."""
    if type(request) is not V121CandidateVerificationRequest:
        raise PipelineError("request must be an exact V121CandidateVerificationRequest")
    if not _reconstructs_exactly(request, V121CandidateVerificationRequest):
    ...
    checks["candidate_contract"] = (
        type(request.contract) is V121CandidateContract
        and all(
    ...
    checks["parent_chain"] = authority_valid["parent"]
    ...
    expected_rollback = {
        "schema_version": "task11-v121-rollback-v1",
        "action": "delete_unpromoted_v121_candidate_tree_if_digest_matches",
    ...
    expected_manifest = {
        "schema_version": "task11-v121-candidate-manifest-v1",
        ...
        "verification": {
            "authority": "verify_v121_candidate",
    ...
    try:
        ...
        releases_v121 = (config.releases_root / "V1.21").resolve(strict=False)
        strict_staging = root != staging and root.is_relative_to(staging)
        outside_formal = not root.is_relative_to(releases_v121)
