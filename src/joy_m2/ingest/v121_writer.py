"""Deterministic Task 11 V1.21 aggregate candidate writer."""
...
from .v121_verification import (
    _authority_expectations,
    _formal_baseline_valid,
    verify_v121_candidate,
)
from . import v121_verification as _independent_verification
...
_BASELINE = {
    ...
    "manifest": {
        "sha256": "19ac2e43ded74a3f3eaf75c184b5eb231c9f1b0b152f5914cd822a321b045098",
        "size_bytes": 6_643,
        "kind": "manifest",
    },
...
def _nested_authority_valid(batch: object) -> bool:
    try:
        ...
        if not all((
            ...
            _reconstructs_exactly(result, V121ImportPreflightResult),
            _reconstructs_exactly(manifest, V121BatchImportManifest),
            _reconstructs_exactly(state, V121EffectiveState),
            _reconstructs_exactly(report, V121ImportPreflightReport),
...
def _collision_authority_valid(
    batches: tuple[V121ApprovedBatch, ...],
    baseline_path: Path,
) -> bool:
    try:
...
def _validate_request(
    request: V121CandidateBuildRequest,
    config: PipelineConfig,
) -> tuple[Path, tuple[V121ApprovedBatch, ...], dict[str, bytes], dict[str, object]]:
    if type(request) is not V121CandidateBuildRequest:
        raise PipelineError("request must be an exact V121CandidateBuildRequest")
    ...
    if not _reconstructs_exactly(request.contract, V121CandidateContract):
        raise PipelineError("contract carrier does not match the fixed V1.21 authority")
    if not _reconstructs_exactly(request, V121CandidateBuildRequest):
    ...
    if (
        ...
        or resolved.is_relative_to(config.releases_root / "V1.21")
    ...
    verification_request = V121CandidateVerificationRequest(output, batches, request.contract)
    if not _formal_baseline_valid(verification_request, output, config):
        raise InputFormatError("formal V1.20 baseline authority is invalid")
...
def _populate_database(
    ...
    try:
        ...
        for statement in TASK11_SCHEMA_SQL[:-1]:
        ...
        for ordinal, batch in enumerate(batches, start=1):
            ...
            connection.execute(
                "INSERT INTO task11_v121_batch_ledger_v1 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    ordinal,
                    approval.batch_id,
                    "V1.21",
                    "task11-v121-candidate-v1",
            ...
            for order, candidate in enumerate(result.candidates):
                ...
                connection.execute(
                    "INSERT INTO task11_v121_candidates_v1 VALUES ("
                    + ",".join("?" for _ in row)
                    + ")",
                    row,
                )
                ...
                for image_order, (source, digest, role, destination) in enumerate(zip(
                    ...
                    connection.execute(
                        "INSERT INTO task11_v121_images_v1 VALUES (?,?,?,?,?,?,?,?,?,?)",
                        (
                            ordinal,
                            approval.batch_id,
                            candidate.proposed_question_id,
            ...
            for kind, values in (("primary_type", primary_types), ("tag", tags)):
                for sort_order, value in enumerate(values, start=1):
                    connection.execute(
                        "INSERT INTO task11_v121_taxonomy_v1 VALUES (?,?,?,?,?,?)",
                        (
                            ordinal,
                            approval.batch_id,
                            kind,
        ...
        connection.execute(TASK11_SCHEMA_SQL[-1])
...
def _ledger_entries(
    batches: tuple[V121ApprovedBatch, ...],
) -> tuple[V121BatchLedgerEntry, ...]:
    cumulative = 0
    result = []
    for ordinal, batch in enumerate(batches, start=1):
        cumulative += len(batch.preflight_result.candidates)
        result.append(V121BatchLedgerEntry(
            ordinal,
...
def build_v121_candidate(
    request: V121CandidateBuildRequest,
    config: PipelineConfig,
) -> V121CandidateArtifacts:
    """Build, independently verify, and atomically publish the first V1.21 prefix."""
    ...
    try:
        ...
        raise InputMissingError("formal V1.20 database cannot be read") from error
    if len(baseline_bytes) != BASELINE_SIZE_BYTES or sha256_bytes(baseline_bytes) != BASELINE_SHA256:
        raise InputFormatError("formal V1.20 database bytes changed during validation")
    ...
    try:
        ...
        verification_request = V121CandidateVerificationRequest(
            temporary, batches, request.contract,
        )
        verification = verify_v121_candidate(verification_request, config)
        if (
        ...
        final_verification = _independent_verification.verify_v121_candidate(
