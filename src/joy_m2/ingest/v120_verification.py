"""Independent read-only V1.20 candidate verification boundary."""
...
def _nested_authority_valid(batch: object) -> bool:
    try:
        ...
        return (
            ...
            and _reconstructs_exactly(result, V120ImportPreflightResult)
            and _reconstructs_exactly(manifest, V120BatchImportManifest)
            and _reconstructs_exactly(state, V120EffectiveState)
            and _reconstructs_exactly(report, V120ImportPreflightReport)
...
def _package_evidence_valid(approved: V120ApprovedBatch) -> bool:
    try:
...
def _candidate_identity(
    ...
    for ordinal, approved in enumerate(approved_batches, start=1):
        ...
        for order, candidate in enumerate(result.candidates):
            ...
            candidates.append({
                "aggregate_order": 502 + prior + order + 1,
                "batch_ordinal": ordinal,
                "batch_id": approval.batch_id,
    ...
    try:
        ...
        for ordinal, approved in enumerate(approved_batches, start=1):
            ...
            expected_statement = (
                ...
                f"{approval.preflight_sha256} V1.20 PARENT {approval.parent_candidate_digest}"
            )
            valid["approval"] &= (
                approval.batch_id == result.manifest.batch_id
                and approval.preflight_sha256 == result.report.preflight_sha256
                and approval.target_release_version == "V1.20"
            ...
            valid["parent"] &= (
                ...
                and result.effective_state.projected_question_count == 502 + prior_count
                ...
                and result.report.before_count == 502 + prior_count
            )
            ...
            prefix = {
                ...
                "counts": {
                    ...
                    "projected_question_count": 502 + prior_count,
                },
            }
...
def _formal_baseline_valid(
    ...
    try:
        ...
        expected_sums = (
            f"{BASELINE_SHA256}  Joy_M2_Complete_Question_DB_V1_19.sqlite3\n"
        ...
        return (
            ...
            and parsed.get("release_version") == "V1.19"
            and parsed.get("release_status") == "published"
            and parsed.get("database_schema") == "task9-v119-formal-v1"
...
def _sqlite_expected(
    ...
    for ordinal, approved in enumerate(approved_batches, start=1):
        ...
        ledgers.append((
            ...
            result.report.manifest_sha256, approval.statement, "V1.19",
            BASELINE_SHA256, BASELINE_RELEASE_DIGEST, 502, len(result.candidates),
            cumulative, 502 + cumulative, "candidate",
        ))
        for order, candidate in enumerate(result.candidates):
...
def _actual_collision_free(
    ...
    try:
        ...
        candidate_rows = tuple(database.execute(
            ...
            "image_roles_json FROM task10_v120_candidates_v1 ORDER BY aggregate_order"
        ))
...
def _sqlite_checks(
    ...
    try:
        ...
        expected_task10 = {}
        for sql in TASK10_SCHEMA_SQL:
            words = sql.split()
            kind = words[1].lower()
            name = words[2]
            expected_task10[name] = (kind, sql)
        actual_task10 = {
            name: row for name, row in candidate_master.items()
            if name.startswith("task10_")
        }
        state["sqlite_schema"] = (
            candidate.execute("PRAGMA user_version").fetchone() == (120,)
            and set(actual_task10) == set(expected_task10)
            and all(
                actual_task10[name][0] == kind
                and normalized_sql(actual_task10[name][3]) == normalized_sql(sql)
                for name, (kind, sql) in expected_task10.items()
            )
            and all(candidate_master.get(name) == row for name, row in baseline_master.items())
            and set(candidate_master) == set(baseline_master) | set(expected_task10)
        )
        preserved_names = tuple(
            name for name, row in baseline_master.items() if row[0] in {"table", "view"}
        )
        state["v119_preservation"] = all(
        ...
        actual_ledger = tuple(candidate.execute(
            "SELECT * FROM task10_v120_batch_ledger_v1 ORDER BY batch_ordinal"
        ))
        ...
        actual_images = tuple(candidate.execute(
            "SELECT * FROM task10_v120_images_v1 ORDER BY candidate_relative_path,"
            "batch_ordinal,batch_id,proposed_question_id,image_order"
        ))
        actual_taxonomy = tuple(candidate.execute(
            "SELECT * FROM task10_v120_taxonomy_v1 ORDER BY batch_ordinal,batch_id,"
        ...
        state["candidate_projection"] = (
            ...
            and tuple(candidate.execute(
                "SELECT * FROM task10_candidate_questions_v120 ORDER BY formal_order"
            ))[:502]
            == tuple(baseline.execute(
                "SELECT * FROM formal_complete_questions_v119 ORDER BY formal_order"
            ))
        )
        ...
        state["count_closure"] = (
            ...
            and candidate.execute(
                "SELECT COUNT(*) FROM task10_v120_candidates_v1"
            ).fetchone() == (identity["counts"]["new_candidate_count"],)
            and candidate.execute(
                "SELECT COUNT(*) FROM task10_candidate_questions_v120"
...
def verify_v120_candidate(
    ...
    """Verify one V1.20 staging generation without mutating any input."""
    if type(request) is not V120CandidateVerificationRequest:
        raise PipelineError("request must be an exact V120CandidateVerificationRequest")
    if not _reconstructs_exactly(request, V120CandidateVerificationRequest):
    ...
    checks["candidate_contract"] = (
        type(request.contract) is V120CandidateContract
        and all(
    ...
    expected_manifest = {
        "schema_version": "task10-v120-candidate-manifest-v1",
    ...
    try:
        ...
        releases_v120 = (config.releases_root / "V1.20").resolve(strict=False)
        strict_staging = root != staging and root.is_relative_to(staging)
        outside_formal = not root.is_relative_to(releases_v120)
