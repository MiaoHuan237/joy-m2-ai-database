"""Deterministic Task 12 V1.22 aggregate candidate writer."""
...
from .v122_verification import (
    _authority_expectations,
    _formal_baseline_valid,
    verify_v122_candidate,
)
from . import v122_verification as _independent_verification
...
_BASELINE = {
    ...
    "manifest": {
        "sha256": "a04f7ee7b67991427bffd3e5a3de70a310d0889fdf8f0535649840a44baf3a40",
        "size_bytes": 7_913,
        "kind": "manifest",
    },
...
def _nested_authority_valid(batch: object) -> bool:
    try:
        ...
        if not all((
            ...
            _reconstructs_exactly(result, V122ImportPreflightResult),
            _reconstructs_exactly(manifest, V122BatchImportManifest),
            _reconstructs_exactly(state, V122EffectiveState),
            _reconstructs_exactly(report, V122ImportPreflightReport),
...
def _collision_authority_valid(
    batches: tuple[V122ApprovedBatch, ...],
    baseline_path: Path,
) -> bool:
    try:
        with sqlite3.connect(f"{baseline_path.resolve().as_uri()}?mode=ro", uri=True) as database:
...
def _validate_request(
    request: V122CandidateBuildRequest,
    config: PipelineConfig,
) -> tuple[Path, tuple[V122ApprovedBatch, ...], dict[str, bytes], dict[str, object]]:
    if type(request) is not V122CandidateBuildRequest:
        raise PipelineError("request must be an exact V122CandidateBuildRequest")
    ...
    if not _reconstructs_exactly(request.contract, V122CandidateContract):
        raise PipelineError("contract carrier does not match the fixed V1.22 authority")
    if not _reconstructs_exactly(request, V122CandidateBuildRequest):
    ...
    if (
        ...
        or resolved.is_relative_to(config.releases_root / "V1.22")
    ...
    verification_request = V122CandidateVerificationRequest(output, batches, request.contract)
    if not _formal_baseline_valid(verification_request, output, config):
        raise InputFormatError("formal V1.21 baseline authority is invalid")
...
def _populate_database(
    ...
    try:
        ...
        for statement in TASK12_SCHEMA_SQL[:-1]:
        ...
        for ordinal, batch in enumerate(batches, start=1):
            ...
            connection.execute(
                "INSERT INTO task12_v122_batch_ledger_v1 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    ordinal,
                    approval.batch_id,
                    "V1.22",
                    "task12-v122-candidate-v1",
            ...
            for order, candidate in enumerate(result.candidates):
                ...
                connection.execute(
                    "INSERT INTO task12_v122_candidates_v1 VALUES ("
                    + ",".join("?" for _ in row)
                    + ")",
                    row,
                )
                ...
                for image_order, (source, digest, role, destination) in enumerate(zip(
                    ...
                    connection.execute(
                        "INSERT INTO task12_v122_images_v1 VALUES (?,?,?,?,?,?,?,?,?,?)",
                        (
                            ordinal,
                            approval.batch_id,
                            candidate.proposed_question_id,
            ...
            for kind, values in (("primary_type", primary_types), ("tag", tags)):
                for sort_order, value in enumerate(values, start=1):
                    connection.execute(
                        "INSERT INTO task12_v122_taxonomy_v1 VALUES (?,?,?,?,?,?)",
                        (
                            ordinal,
                            approval.batch_id,
                            kind,
        ...
        connection.execute(TASK12_SCHEMA_SQL[-1])
...
def _ledger_entries(
    batches: tuple[V122ApprovedBatch, ...],
) -> tuple[V122BatchLedgerEntry, ...]:
    cumulative = 0
    result = []
    for ordinal, batch in enumerate(batches, start=1):
        cumulative += len(batch.preflight_result.candidates)
        result.append(V122BatchLedgerEntry(
            ordinal,
...
def build_v122_candidate(
    request: V122CandidateBuildRequest,
    config: PipelineConfig,
) -> V122CandidateArtifacts:
    """Build, independently verify, and atomically publish a V1.22 prefix."""
    output, batches, authority_bytes, identity = _validate_request(request, config)
    genesis = _genesis_digest()
    if genesis != "7f449c4428900537f99a7118ed702da462afa0f00ae1aa38e5296dacf6099dd7":
        raise ImportApprovalError("V1.22 reconstructed genesis is not the frozen authority")
    first = request.approved_batches[0]
    if (first.approval.parent_candidate_digest != genesis
        or first.preflight_result.report.parent_candidate_digest != genesis
        or first.preflight_result.effective_state.candidate_digest != genesis):
        raise ImportApprovalError("first batch must bind independently reconstructed genesis")
    ...
    try:
        ...
        raise InputMissingError("formal V1.21 database cannot be read") from error
    if len(baseline_bytes) != BASELINE_SIZE_BYTES or sha256_bytes(baseline_bytes) != BASELINE_SHA256:
        raise InputFormatError("formal V1.21 database bytes changed during validation")
    ...
    try:
        ...
        verification_request = V122CandidateVerificationRequest(
            temporary, batches, request.contract,
        )
        verification = verify_v122_candidate(verification_request, config)
        if (
        ...
        final_verification = _independent_verification.verify_v122_candidate(
