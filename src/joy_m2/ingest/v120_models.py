"""Immutable public authority models for Task 10A V1.20 candidates."""
...
_GENESIS_DIGEST = "4ff624aca875b0191fe8a617516d2919c6d700ce69c5a3b5d236adcf7ecc59f1"
_BASELINE = {
    "release_version": "V1.19",
    "question_count": 502,
    "database_sha256": "5a7f1ca01c29dc638fb592c668ea9f597551f94d73001898587b187bdbe490ff",
    "database_size_bytes": 9478144,
    "manifest_sha256": "cd048408df5e062592d5ed8553b3461860c876b0264660ab65c78c428584a510",
    "manifest_size_bytes": 3242,
...
_CANDIDATE_VIEWS = ("task10_candidate_questions_v120",)
_REPORT_STATUSES = {
...
def _require_batch_id(value: object, name: str = "batch_id") -> str:
    if type(value) is not str or _BATCH_ID.fullmatch(value) is None:
        raise PipelineError(f"{name} must be a canonical V1.20 batch identifier")
    return value
...
def _baseline_artifact(value: object, name: str) -> ArtifactRef:
    ...
    if (
        ...
        raise PipelineError(f"{name} does not match the V1.19 baseline authority")
    return artifact


def _validate_report_tuples(report: "V120ImportPreflightReport") -> None:
    for name in (
...
class V120AdaptedImportPackage:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.manifest) is not V120BatchImportManifest:
            raise PipelineError("manifest must be an exact V120BatchImportManifest")
...
class V120ImportApproval:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.target_release_version) is not str or self.target_release_version != "V1.20":
            raise ImportApprovalError("target_release_version must equal V1.20")
        expected = (
            f"USER APPROVED IMPORT BATCH {self.batch_id} {self.preflight_sha256} "
            f"V1.20 PARENT {self.parent_candidate_digest}"
        )
...
class V120BatchLedgerEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.approval) is not V120ImportApproval:
            raise PipelineError("approval must be an exact V120ImportApproval")
        if (
            self.approval.batch_id != self.batch_id
            or self.approval.preflight_sha256 != self.preflight_sha256
            or self.approval.target_release_version != "V1.20"
        ...
        if self.projected_question_count != 502 + self.cumulative_candidate_count:
...
class V120EffectiveState:
    ...
    def __post_init__(self) -> None:
        ...
        ledger = _exact_tuple(self.batch_ledger, V120BatchLedgerEntry, "batch_ledger")
        ...
        if not ledger:
            if (
                ...
                or self.projected_question_count != 502
            ):
                raise PipelineError("empty V1.20 state must be the fixed genesis state")
        else:
            previous_count = 0
        ...
        if self.projected_question_count != 502 + self.candidate_count:
    ...
    def __post_init__(self) -> None:
        for name, expected in _CONTRACT_VALUES.items():
            ...
            if type(value) is not type(expected) or value != expected:
                raise PipelineError(f"{name} does not match the V1.20 candidate contract")
        ...
        if tables != _CANDIDATE_TABLES or views != _CANDIDATE_VIEWS:
            raise PipelineError("candidate contract object tuples do not match V1.20")
...
class V120PreflightRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "package_root", _path(self.package_root, "package_root"))
        _baseline_artifact(self.baseline_database, "baseline_database")
        if self.parent_candidate is not None and type(self.parent_candidate) is not V120CandidateVerificationRequest:
            raise PipelineError("parent_candidate must be V120CandidateVerificationRequest or None")
        if type(self.contract) is not V120CandidateContract:
            raise PipelineError("contract must be an exact V120CandidateContract")


@dataclass(frozen=True)
class V120ImportPreflightReport:
    batch_id: str
    status: str
    preflight_sha256: str
    manifest_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or type(self.baseline_question_count) is not int or self.baseline_question_count != 502
            or type(self.target_release_version) is not str or self.target_release_version != "V1.20"
        ):
        ...
        for name in count_names:
            ...
            if type(value) is not int or value < 0:
                raise PipelineError(f"{name} must be a non-negative integer")
        if self.before_count < 502:
            raise PipelineError("before_count cannot precede the V1.19 baseline")
...
class V120ImportPreflightResult:
    ...
    def __post_init__(self) -> None:
        if type(self.manifest) is not V120BatchImportManifest:
            raise PipelineError("manifest must be an exact V120BatchImportManifest")
        if type(self.effective_state) is not V120EffectiveState:
            raise PipelineError("effective_state must be an exact V120EffectiveState")
        if type(self.report) is not V120ImportPreflightReport:
            raise PipelineError("report must be an exact V120ImportPreflightReport")
        if (
...
class V120ApprovedBatch:
    ...
    def __post_init__(self) -> None:
        if type(self.preflight_result) is not V120ImportPreflightResult:
            raise PipelineError("preflight_result must be an exact V120ImportPreflightResult")
        object.__setattr__(self, "package_root", _path(self.package_root, "package_root"))
        if type(self.approval) is not V120ImportApproval:
            raise PipelineError("approval must be an exact V120ImportApproval")
...
class V120CandidateBuildRequest:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "approved_batches", _exact_tuple(
            self.approved_batches, V120ApprovedBatch, "approved_batches", nonempty=True,
        ))
...
class V120CandidateArtifacts:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.state) is not V120EffectiveState:
            raise PipelineError("state must be an exact V120EffectiveState")
