"""Strict V1.21 canonical package-manifest boundary."""
...
from .v121_models import V121BatchImportManifest


__all__ = ("load_v121_import_manifest",)
...
def load_v121_import_manifest(path: Path) -> V121BatchImportManifest:
    """Load one strict V1.21 package manifest from its canonical package root."""
    ...
    expected_fields = tuple(field.name for field in fields(V121BatchImportManifest))
    ...
    try:
        return V121BatchImportManifest(**values)
    except (TypeError, PipelineError) as exc:
        raise PipelineError("manifest does not satisfy V1.21 authority") from exc
