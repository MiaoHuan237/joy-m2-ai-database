"""Private deterministic constants and pure helpers for Task 12 candidates."""
...
TASK12_SCHEMA_SQL = (
    """CREATE TABLE task12_v122_batch_ledger_v1 (
    ...
    target_release_version TEXT NOT NULL CHECK(target_release_version = 'V1.22'),
    candidate_database_schema TEXT NOT NULL
        CHECK(candidate_database_schema = 'task12-v122-candidate-v1'),
    ...
    projected_question_count INTEGER NOT NULL
        CHECK(projected_question_count = 591 + cumulative_candidate_count),
    record_status TEXT NOT NULL CHECK(record_status = 'candidate'),
    UNIQUE(batch_ordinal, batch_id)
)""",
    """CREATE TABLE task12_v122_candidates_v1 (
    ...
    aggregate_order INTEGER NOT NULL CHECK(aggregate_order >= 592),
    ...
    """CREATE TABLE task12_v122_images_v1 (
    batch_ordinal INTEGER NOT NULL CHECK(batch_ordinal >= 1),
    batch_id TEXT NOT NULL,
    proposed_question_id TEXT NOT NULL,
    ...
    FOREIGN KEY(batch_ordinal, batch_id, proposed_question_id)
        REFERENCES task12_v122_candidates_v1(
            batch_ordinal, batch_id, proposed_question_id
        )
)""",
    """CREATE TABLE task12_v122_taxonomy_v1 (
    batch_ordinal INTEGER NOT NULL CHECK(batch_ordinal >= 1),
    batch_id TEXT NOT NULL,
    ...
    """CREATE VIEW task12_candidate_questions_v122 AS
SELECT f.*
FROM formal_complete_questions_v121 AS f
UNION ALL
SELECT
    c.aggregate_order AS formal_order,
    c.proposed_question_id AS question_id,
    'task12_v122_candidate' AS authority_kind,
    ...
    c.record_status,
    c.selectable
FROM task12_v122_candidates_v1 AS c
...
def _genesis_digest() -> str:
    """Reconstruct the fixed first-parent authority, never from artifacts."""
    projection = {
