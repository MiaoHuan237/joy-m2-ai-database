...
准确验收口径为：“Task 8A 设计与计划阶段通过；新增测试延期至 Task 8B。”

Task 8A 已完成已批准的文档阶段范围：调查 legacy Task 5/6、确认设计、提交设计规格、在书面批准后生成实施计划，并保持生产代码、测试树和冻结数据不变。当前分支没有新增行为锁定测试或接口契约测试：两次测试发现均为 `Ran 0 tests`，退出码为 5。0 项是延期状态，不代表测试通过；测试创建与执行属于 Task 8B。

既有门禁、冻结哈希、SQLite 不变量、Git tree 和范围边界全部通过。
...
- Task 8A 定位为统一数据流水线接口设计与实施计划阶段，不是测试实现阶段。
- 新增行为锁定测试为 0 项，延期至 Task 8B，不得写成 PASS。
- pipeline 接口契约测试为 0 项，延期至 Task 8B，不得写成 PASS。
- Task 8A 不需要越界补写测试；创建或执行这些测试将进入 Task 8B，必须另行授权。

以下“范围与后续动作”记录的是验收命令执行并生成本报告时的状态；后续 push 和 PR 状态以 GitHub PR #1 为准。

## 7. 范围与后续动作

- 未合并 `main`。
- 验收命令执行时尚未推送或创建 PR；其后已推送分支并创建草稿 PR #1。
- 未开始 Task 8B。
- 未开始 Task 8C。
