...
append-only root-surface assertions in the three existing V1.19 historical
contract tests; all V1.19 and V1.20 names/order remain byte-for-byte identical.
...
python -m unittest -v tests.unit.test_v119_writer_models tests.unit.test_v119_promotion_models tests.regression.test_v119_historical_replay
```

- [ ] **Step 6: Commit the model checkpoint**

```bash
git add pyproject.toml src/joy_m2/ingest/hkdse_pdf_models.py src/joy_m2/ingest/__init__.py tests/unit/test_hkdse_pdf_models.py tests/unit/test_v119_writer_models.py tests/unit/test_v119_promotion_models.py tests/regression/test_v119_historical_replay.py
...
The explicit module enumeration is mandatory because the test subdirectories
are namespace-style and plain recursive discovery collects zero tests in the
approved runtime.
